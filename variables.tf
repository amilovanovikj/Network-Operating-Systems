...
variable "ssh_key_client" {
  type        = string
  sensitive   = true
  description = "The SSH key for connecting to the Linux client machine. Generate a new one with ssh-keygen."
}
variable "ssh_key_mail" {
  type        = string
  sensitive   = true
  description = "The SSH key for connecting to the FTP/Mail Linux server. Generate a new one with ssh-keygen."
}
